...
Твоє завдання: знайти у тексті менеджера ВСІ згадані готелі і за необхідності виправити їх назви за допомогою списку з бази.

ПРАВИЛА:
1. ПОРЯДОК ТА КІЛЬКІСТЬ: Повертай готелі СУВОРО в тому порядку, в якому вони йдуть у тексті.
2. КІЛЬКІСТЬ: Скільки готелів назвав менеджер — рівно стільки ж має бути у твоїй відповіді.
3. ВИПРАВЛЕННЯ ПОМИЛОК: Використовуй "СПИСОК ГОТЕЛІВ", щоб виправляти ФОНЕТИЧНІ помилки розпізнавання голосу (наприклад, "аш ем мартиник" -> "HM Martinique").
4. СУВОРА ЗАБОРОНА НА ГАЛЮЦИНАЦІЇ: Якщо менеджер ЧІТКО називає готель (наприклад, "Hotel Riu Playa Park"), але в базі є лише інший готель зі схожими словами (наприклад, "4R Playa Park"), КАТЕГОРИЧНО ЗАБОРОНЕНО замінювати його! Ти не повинен "підганяти" назву під базу, якщо це явно інший готель або інша мережа.
5. [NOT_FOUND]: Якщо готелю об'єктивно НЕМАЄ в базі (це не просто помилка в написанні, а інший готель), поверни оригінальну ОЧИЩЕНУ назву з тексту, обов'язково додавши префікс [NOT_FOUND]. Наприклад: "[NOT_FOUND] Hotel Riu Playa Park".
6. ОЧИЩЕННЯ: Очищай назви від цін, типів харчування та сміття. Тільки назва готелю.
...
async def format_tour_message(user_text: str, do_cleanup: bool = False, raw_voice_text: str = None) -> str:
    ...
    fast_models = ["openai/gpt-4o-mini", "google/gemini-2.5-flash"]
    smart_models = ["openai/gpt-4o-mini", "google/gemini-2.5-flash"]
